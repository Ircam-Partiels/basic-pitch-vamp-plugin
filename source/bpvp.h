...
namespace Bpvp
{
    ...
    private:
        ...
        std::array<float, modelBlockSize> mBuffer;
        std::array<float, modelTensorSize> mFrames;
        std::array<float, modelTensorSize> mOnsets;
        std::vector<float> mAccumulatedFrames;
        std::vector<float> mAccumulatedOnsets;
        size_t mBufferPosition{0};
        size_t mBlockSize{0};
        float mNoteSensibility{0.7f};
        float mOnsetSensibility{0.5f};
    };
} // namespace Bpvp
